...
% Perceptual learning has also been demonstrated in the inferotemporal region (IT) for face classification tasks in monkeys \cite{Sigala2002}.
...
A second method of correcting for the bias which makes use of the power series expansion is \iac{QE} method \cite{Strong1998}.
...
Both \iac{PT} and \ac{QE} bias correction methods give similar approximations to the true information, whilst \ac{NBS} outperforms them with a less biased estimate \cite{Panzeri2007}.
However, \ac{NBS} is very computationally intensive \cite{Panzeri2007}, so we will not be making use of it in the presented body work.
