...
Above, traces of $d'$ for each channel (colours), and their average (black).
Below, heatmap showing $d'$ for each channel.
Right top, violin plots showing distribution over channels of the average $d'$ in the first (\zonename{A}) and last (\zonename{B}) three sessions, with mean (solid black line) and median (dashed green line) over channels.
The violin plot shows a Gaussian kernel density using a bandwidth determined as described in \autoref{sec:violin_plot_method}.
...
In both cases, we measured the correlation between the responses from the two channels using the Pearson correlation coefficient, which we introduced in \autoref{eq:pearson-dummy} and restate here.
...
.\end{equation}
This provides a measure of the covariance between $X$ and $Y$ which is normalised again their standard deviations, meaning that $-1 \le \rho \le 1$ and $\rho$ is robust against linear rescaling of either $X$ or $Y$ (or both).
If $\rho=\pm1$, there is a perfect linear relationship between $X$ and $Y$, whereas $\rho=0$ when $X$ and $Y$ are completely independent of one another.
...
For \ac{V1}, the Pearson's correlation coefficient (see \autoref{eq:pearson}) between them was $\rho(\I, \nicefrac{1}{N})=0.99$ and $\rho=0.98$ for \ac{M1} and \ac{M2} respectively, which was a significant correlation in all cases ($p < 2 \times 10^{-13}$ and $p < 8 \times 10^{-16}$).
For \ac{V4}, $\rho=0.98$ and $\rho=0.92$ with p-values $p < 2 \times 10^{-14}$ and $p < 2 \times 10^{-10}$.
But even if we correct for the bias with \ac{PT} or \ac{QE}, the correlation remains large ($\rho>0.4$) and significant ($p<0.04$) for all datasets except \ac{M2} \ac{V1} with \ac{PT}, where $\rho=+0.27$ and $p=0.23$.
The correlation is strongest for \ac{M1} \ac{V1}, with $\rho>0.89$ and $p < 1 \times 10^{-6}$ with either \ac{PT} or \ac{QE} bias correction.
...
This effect is particularly problematic for the \ac{V1} dataset of \ac{M2}, where the correlation was $\rho<-0.72$ ($p < 2 \times 10^{-4}$; see \autoref{fig:I_vs_invN_btsp_v1_jack}), and the \ac{V4} \ac{M1} dataset, where the correlation was $\rho<-0.44$ ($p<0.038$; see \autoref{fig:I_vs_invN_btsp_v4_blanco}) even with bias correction with \ac{PT} or \ac{QE} in addition to using bootstrapping.
...
Here we find the magnitude of the correlations between $\nicefrac{1}{N}$ and $\I_\text{measured}$ are reduced and no longer significant, with the exception of \ac{M2} \ac{V1}, where $\rho<-0.47$ for both \ac{PT} and \ac{QE} ($p < 0.027$).
...
Using all three bias reduction techniques (including either \ac{PT} or \ac{QE}), the correlation for \ac{M2} \ac{V1} was still significant ($p < 0.008$), with $\rho<-0.54$.
...
For this dataset, there was a correlation between the number of sessions elapsed and the number of trials in the session of $\rho=+0.42$, which was noteworthy but did not exceed our criteria for significance ($p=0.053$).
None of the other datasets had a comparable level of correlation between the number of the session and how many trials were collected ($|\rho|<0.22$ with $p > 0.4$).
...
figs/info2/binlen/I_binlength_jack_v4_chmean18_s24,25,27-38,40-49_oc0_Gclass-group-balanced_1bins_4-fold-sampling_dr_pt_btsp20_rmvet2_rmvms2_imscn_clhot.eps}}
    \hspace*{\fill}
    \caption{
    \captionemph{Duration of the window over which firing rate is measured influences the measured information.}
...
Main panels: heatmap showing information against experimental session and window duration (non-significant values shown in white; median significance threshold indicated by a line across the colour bar).
...
\begin{figure}[tbp]
    ...
    \subfloat[][\ac{M1} \ac{V1}, session \num{1}.\label{fig:pl_lin_discrim_example_v1_blanco_pre}]{%
        \includegraphics[scale=.32]{figs/decoding/lindiscrimexLCH-RG-log_cbl_v1_blanco_343_31vs44.eps}}
    \hspace*{\fill}\hspace{.4cm}\hspace*{\fill}
    \subfloat[][\ac{M2} \ac{V1}, session \num{1}.\label{fig:pl_lin_discrim_example_v1_jack_pre}]{%
        \includegraphics[scale=.32]{figs/decoding/lindiscrimexLCH-RG-log_cbl_v1_jack_51_12vs19.eps}}
    %\hspace*{\fill}
    \\
    %\hspace*{\fill}
    \subfloat[][\ac{M1} \ac{V1}, session \num{17}.\label{fig:pl_lin_discrim_example_v1_blanco_post}]{%
        \includegraphics[scale=.32]{figs/decoding/lindiscrimexLCH-RG-log_cbl_v1_blanco_359_31vs44.eps}}
    \hspace*{\fill}\hspace{.4cm}\hspace*{\fill}
    \subfloat[][\ac{M2} \ac{V1}, session \num{22}.\label{fig:pl_lin_discrim_example_v1_jack_post}]{%
        \includegraphics[scale=.32]{figs/decoding/lindiscrimexLCH-RG-log_cbl_v1_jack_72_12vs19.eps}}
    %\hspace*{\fill}
    \caption{
...
\begin{figure}[tbp]
    ...
    \subfloat[][\ac{M1} \ac{V4}, session \num{1}.\label{fig:pl_lin_discrim_example_v4_blanco_pre}]{%
        \includegraphics[scale=.32]{figs/decoding/lindiscrimexLCH-RG-log_cbl_v4_blanco_307_12vs51.eps}}
    \hspace*{\fill}\hspace{.4cm}\hspace*{\fill}
    \subfloat[][\ac{M2} \ac{V4}, session \num{1}.\label{fig:pl_lin_discrim_example_v4_jack_pre}]{%
        \includegraphics[scale=.32]{figs/decoding/lindiscrimexLCH-RG-log_cbl_v4_jack_24_10vs53.eps}}
    %\hspace*{\fill}
    \\
    %\hspace*{\fill}
    \subfloat[][\ac{M1} \ac{V4}, session \num{35}.\label{fig:pl_lin_discrim_example_v4_blanco_post}]{%
        \includegraphics[scale=.32]{figs/decoding/lindiscrimexLCH-RG-log_cbl_v4_blanco_341_12vs51.eps}}
    \hspace*{\fill}\hspace{.4cm}\hspace*{\fill}
    \subfloat[][\ac{M2} \ac{V4}, session \num{25}.\label{fig:pl_lin_discrim_example_v4_jack_post}]{%
        \includegraphics[scale=.32]{figs/decoding/lindiscrimexLCH-RG-log_cbl_v4_jack_48_10vs53.eps}}
    %\hspace*{\fill}
    \caption{
...
\vspace*{-4mm}
    ...
    \subfloat[][\ac{M1} \ac{V1}.\label{fig:dec_b1_allp}]{
        \centering
	\includegraphics[scale=.36]{figs/decoding/perf_percentage_v1_blanco_leg.eps}
    }
    \hspace*{\fill}\hspace{.4cm}\hspace*{\fill}
    \subfloat[][\ac{M2} \ac{V1}.\label{fig:dec_j1_allp}]{
        \centering
	\includegraphics[scale=.36]{figs/decoding/perf_percentage_v1_jack.eps}
    }
    \hspace*{\fill}
    \\
    \hspace*{\fill}
    \subfloat[][\ac{M1} \ac{V1}.\label{fig:dec_b1_info}]{
        \centering
	\includegraphics[scale=.36]{figs/decoding/perf_information_v1_blanco.eps}
    }
    \hspace*{\fill}\hspace{.4cm}\hspace*{\fill}
    \subfloat[][\ac{M2} \ac{V1}.\label{fig:dec_j1_info}]{
        \centering
	\includegraphics[scale=.36]{figs/decoding/perf_information_v1_jack.eps}
    }
    \hspace*{\fill}
    \caption{
    ...
    Also shown is the accuracy of a linear decoder trained on shuffled data, with responses from each channel matched at random, such that noise correlations are removed (red; see \autoref{sec:pl_dec_shuffle}).
    For comparison, the behavioural performance of the subject is also shown (black).
...
\begin{figure}[tbp]
    ...
    \subfloat[][\ac{M1} \ac{V4}.\label{fig:dec_b4_allp}]{
        \centering
	\includegraphics[scale=.36]{figs/decoding/perf_percentage_v4_blanco_leg.eps}
    }
    \hspace*{\fill}\hspace{.4cm}\hspace*{\fill}
    \subfloat[][\ac{M2} \ac{V4}.\label{fig:dec_j4_allp}]{
        \centering
	\includegraphics[scale=.36]{figs/decoding/perf_percentage_v4_jack.eps}
    }
    \hspace*{\fill}
    \\
    \hspace*{\fill}
    \subfloat[][\ac{M1} \ac{V4}.\label{fig:dec_b4_info}]{
        \centering
	\includegraphics[scale=.36]{figs/decoding/perf_information_v4_blanco.eps}
    }
    \hspace*{\fill}\hspace{.4cm}\hspace*{\fill}
    \subfloat[][\ac{M2} \ac{V4}.\label{fig:dec_j4_info}]{
        \centering
	\includegraphics[scale=.36]{figs/decoding/perf_information_v4_jack.eps}
    }
    \hspace*{\fill}
    \caption{%
...
The intersection between all three circles (grey region) is $\I({X;Y;Z})$, the entropy (or information) mutually shared by all three variables.
...
\begin{figure}[tbp]
    ...
    \subfloat[][\ac{M1} \ac{V1}.\label{fig:decag_b1_allp}]{
        \centering
	\includegraphics[scale=.36]{figs/decoding/agree_percentage_v1_blanco.eps}
    }
    \hspace*{\fill}\hspace{.4cm}\hspace*{\fill}
    \subfloat[][\ac{M2} \ac{V1}.\label{fig:decag_j1_allp}]{
        \centering
	\includegraphics[scale=.36]{figs/decoding/agree_percentage_v1_jack_leg.eps}
    }
    \hspace*{\fill}
    \\
    \hspace*{\fill}
    \subfloat[][\ac{M1} \ac{V1}.\label{fig:decag_b1_condinfo}]{
        \centering
	\includegraphics[scale=.36]{figs/decoding/agree_conditional-information_v1_blanco.eps}
    }
    \hspace*{\fill}\hspace{.4cm}\hspace*{\fill}
    \subfloat[][\ac{M2} \ac{V1}.\label{fig:decag_j1_condinfo}]{
        \centering
	\includegraphics[scale=.36]{figs/decoding/agree_conditional-information_v1_jack.eps}
    }
    \hspace*{\fill}
    \caption{
...
\begin{figure}[tbp]
    ...
    \subfloat[][\ac{M1} \ac{V4}.\label{fig:decag_b4_allp}]{
        \centering
	\includegraphics[scale=.36]{figs/decoding/agree_percentage_v4_blanco_leg.eps}
    }
    \hspace*{\fill}\hspace{.4cm}\hspace*{\fill}
    \subfloat[][\ac{M2} \ac{V4}.\label{fig:decag_j4_allp}]{
        \centering
	\includegraphics[scale=.36]{figs/decoding/agree_percentage_v4_jack.eps}
    }
    \hspace*{\fill}
    \\
    \hspace*{\fill}
    \subfloat[][\ac{M1} \ac{V4}.\label{fig:decag_b4_condinfo}]{
        \centering
	\includegraphics[scale=.36]{figs/decoding/agree_conditional-information_v4_blanco.eps}
    }
    \hspace*{\fill}\hspace{.4cm}\hspace*{\fill}
    \subfloat[][\ac{M2} \ac{V4}.\label{fig:decag_j4_condinfo}]{
        \centering
	\includegraphics[scale=.36]{figs/decoding/agree_conditional-information_v4_jack.eps}
    }
    \hspace*{\fill}
    \caption{
