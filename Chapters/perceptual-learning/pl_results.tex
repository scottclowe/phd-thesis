...
To investigate whether $d'$ changed significantly during the course of our experiments, we compared the average $d'$ during the first 3 and final 3 experimental sessions.
...
The average change in was $\Delta d' = -0.524267$ ($p < 3e-04$, paired $t$-test) for \ac{M1} and $\Delta d' = -0.463924$ ($p < 3e-10$, paired $t$-test) for \ac{M2}.
...
figs/dprime/dprime_sample_hm_hotcold_blanco_v1_1_rmon2.eps}
    }
    \hspace*{\fill}\hspace{.2cm}\hspace*{\fill}
    \subfloat[\ac{M2} \ac{V1}\label{fig:dprime_v1_jack}]{
        \centering
        \includegraphics[scale=.45]{%
figs/dprime/dprime_sample_hm_hotcold_jack_v1_1_rmon2.eps}
    }
    \hspace*{\fill}
...
Right top, violin plots showing distribution over channels of the average $d'$ in the first 3 sessions (A) and last 3 sessions (B), with mean (solid black line) and median (dashed green line) over channels indicated.
The violin plot shows a Gaussian kernel density using a bandwidth of $0.11$.
...
Overall, there was a small decrease in average $d'$, with $\Delta d' = -0.094373$, which was not a statistically significant change ($p=0.25$).
...
Starting from this lower position, there was a significant increase of $\Delta d' = +0.340810$ ($p<5e-07$).
...
\begin{figure}[htbp]
    ...
    \subfloat[\ac{M1} \ac{V4}\label{fig:dprime_v4_blanco}]{
        \centering
        \includegraphics[scale=.45]{%
figs/dprime/dprime_sample_hm_hotcold_blanco_v4_1_rmon2.eps}
    }
    \hspace*{\fill}\hspace{.2cm}\hspace*{\fill}
    \subfloat[\ac{M2} \ac{V4}\label{fig:dprime_v4_jack}]{
        \centering
        \includegraphics[scale=.45]{%
figs/dprime/dprime_sample_hm_hotcold_jack_v4_1_rmon2.eps}
    }
    \hspace*{\fill}
...
Overtime, the noise increases and the signal-to-noise ratio falls, which leads to a reduction in the distinguisability of the two activity distributions.
...
The mutual information between the spiking activity during test presentation and the identity of the test stimulus was computed using a spike-timing based code.
This computation was done using the \verb|information| function from the freely available \textit{Information Breakdown Toolbox} \citep{Magri2009} for MATLAB.%
...
\begin{figure}[htbp]%
    \centering
    \hspace*{\fill}
    \subfloat[][\ac{M1} \ac{V1}.\label{fig:info_sess_v1_blanco}]{%
        \includegraphics[scale=.45]{%
figs/info2/I_sessionwise_blanco_v1_chmean23_s343-359_oc0_1bins_of_20ms_dr_pt_imscn_clhot.eps}}
    \hspace*{\fill}\hspace{.2cm}\hspace*{\fill}
    \subfloat[][\ac{M1} \ac{V4}.\label{fig:info_sess_v1_jack}]{%
        \includegraphics[scale=.45]{%
figs/info2/I_sessionwise_jack_v1_chmean25_s51-72_oc0_1bins_of_20ms_dr_pt_imscn_clhot.eps}}
    \hspace*{\fill}
    \caption{Information about the test stimulus contained in the neural activity during test presentation, as a function of number of .
...
In each case the average is taken across all available channel data: \protect\subref{fig:info_sess_v1_blanco}~23 channels, \protect\subref{fig:info_sess_v4_blanco}~30 channels, \protect\subref{fig:info_sess_v1_jack}~25 channels, \protect\subref{fig:info_sess_v4_jack}~20 channels.
The \ac{PT} bias correction method was used (see text).
}
    \label{fig:info_sess_v1}
\end{figure}
...
\begin{figure}[htbp]%
    \centering
    \hspace*{\fill}
    \subfloat[][\ac{M1} \ac{V1}.\label{fig:info_sess_v4_blanco}]{%
        \includegraphics[scale=.45]{%
figs/info2/I_sessionwise_blanco_v4_chmean30_s307,308,311,313,314,317,318,320,321,329-341_oc0_1bins_of_20ms_dr_pt_imscn_clhot.eps}}
    \hspace*{\fill}\hspace{.2cm}\hspace*{\fill}
    \subfloat[][\ac{M1} \ac{V4}.\label{fig:info_sess_v4_jack}]{%
        \includegraphics[scale=.45]{%
figs/info2/I_sessionwise_jack_v4_chmean20_s24,25,27-38,40-49_oc0_1bins_of_20ms_dr_pt_imscn_clhot.eps}}
    \hspace*{\fill}
...
The \ac{PT} bias correction method was used (see text).
    ...
    \label{fig:info_sess_v4}
\end{figure}




The results of this initial analysis are shown in \autoref{fig:sess}.
...
\autoref{fig:IandN} shows that the changes in the measured mutual information are dominated by the number of trials in the session, not by increases with perceptual learning (corresponding to increments in the session number).
...
% 
\begin{figure}[htbp]
    \subfloat[][\ac{M1} \ac{V1}\label{fig:IvNb1}]{
        \centering
        \includegraphics[width=0.47\linewidth]{%
figs/info/ntrialsIvsinvNcombindiv_blanco_v1_5bins_of_4ms_20120815T204808.eps}
    }
    \subfloat[][\ac{M1} \ac{V4}\label{fig:IvNb4}]{
        \centering
        \includegraphics[width=0.47\linewidth]{%
figs/info/ntrialsIvsinvNcombindiv_blanco_v4_5bins_of_4ms_20120815T204808.eps}
    }
    \\
    \subfloat[][\ac{M2} \ac{V1}\label{fig:IvNj1}]{
        \centering
        \includegraphics[width=0.47\linewidth]{%
figs/info/ntrialsIvsinvNcombindiv_jack_v1_5bins_of_4ms_20120815T204808.eps}
    }
    \subfloat[][\ac{M2} \ac{V4}\label{fig:IvNj4}]{
        \centering
        \includegraphics[width=0.47\linewidth]{%
figs/info/ntrialsIvsinvNcombindiv_jack_v4_5bins_of_4ms_20120815T204808.eps}
    }
...
In particular, the measured value for the mutual information is strongly correlated with the reciprocal of the number of trials in the session (\autoref{fig:IvN}).
