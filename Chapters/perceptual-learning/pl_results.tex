...
We used \autoref{eq:estimate-bw} to find a suitable kernel basis bandwidth for the distribution over channels of information at the start ($A$) and end ($B$) of training.
...
To test the significance of changes in information over time, we used a paired Student's $t$-test to compare the difference in information values in $\SET{A}$ and $\SET{B}$ against the null-hypothesis of no change between points $A$ and $B$.
...
We could instead have used the Mann--Whitney $U$ test to compare the two distributions $\SET{A}$ and $\SET{B}$.
...
Since we record from the same set of channels for both $\SET{A}$ and $\SET{B}$, we are violating the independence assumption, and so the paired Student's $t$-test is a more appropriate choice.
...
Right hand panels: distribution over channels of the information contained in the first three sessions (A) versus last three sessions (B), with mean (solid black line) and median (dashed green line) over channels indicated.
...
Right hand panels: distribution over channels of the information contained in the first three blocks of $1400$ trials (A) versus last three blocks (B), with mean (solid black line) and median (dashed green line) over channels indicated.
The violin plot shows a Gaussian kernel density, using a bandwidth determined as described in \autoref{sec:info-methods}.
The \ac{PT} bias correction method was used, without further correction to the residual bias.
