...
\begin{figure}[tbhp]
\includegraphics[width=1.05\linewidth]{figs/task/PLtask1.pdf}
\caption{
...
Centre co-ords (\si{\dva})  & $(-5, 16)$        & $(-3.5, 3)$       & $(-5, 16)$        & $(-0.7, -1.3)$    \\
Eccentricity (\si{\dva})    & \SI{16.8}{\degree}& \SI{4.6}{\degree} & \SI{16.8}{\degree}& \SI{1.48}{\degree}\\
Stimulus size (\si{\dva})   & \SI{16.0}{\degree}& \SI{3.0}{\degree} & \SI{14.0}{\degree}& \SI{0.75}{\degree}\\
Stimulus type               & Gabor             & sinusoid          & Gabor             & sinusoid          \\
Spatial frequency (\si{\cpd}) & \num{2}         & \num{2}           & \num{2}           & \num{4}           \\
\bottomrule
%
\end{tabular}
} % end centerline
...
Stimulus co-ordinates are given in \aclu{dva} (\si{\dva}).
Spatial frequency is specified in \aclu{cpd} (\si{\cpd}).
}
\label{tab:pl_stim2}
\end{table}
