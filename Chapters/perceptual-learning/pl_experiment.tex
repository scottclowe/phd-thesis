...
All experimental proceedings described in this section where performed by Xing Chen.
...
\begin{table}[bthp]
\caption{
Stimuli parameters for each subject and recording region.
The set of test contrasts were selected to so the difficulty of the task ranged from easy to very hard.
...
\midrule
M1      & V4    & Gabor     & 10, 15, 20, 25, 27, 28, 29, 31, 32, 33, 35, 40, 50, 60 \\
        & V1    & sinusoid  &  5, 10, 15, 20, 22, 25, 28, 32, 35, 40, 45, 50, 60, 90 \\
M2      & V4    & Gabor     & 10, 15, 20, 25, 27, 28, 29, 31, 32, 33, 35, 40, 50, 60 \\
        & V1    & sinusoid  &  5, 10, 15, 20, 22, 25, 28, 32, 35, 40, 45, 50, 60, 90 \\
\bottomrule
%
\end{tabular}
...
\begin{table}[bthp]
\caption{
Experimental details for each animal and \ac{MEA} recording region.
}
\label{tab:pl_stim2}
\centerline{
\begin{tabular}{lcccc}
...
Centre coords (dva)     & $(-5, 16)$    & $(-3.5, 3)$   & $(-5, 16)$    & $(-0.7, -1.3)$\\
Size (dva)              & 16            & 3             & 14            & 0.75          \\
Stimulus type           & Gabor         & sinusoid      & Gabor         & sinusoid      \\
Spatial frequency (cpd) & 2             & 2             & 2             & 4             \\
\bottomrule
%
\end{tabular}
} % end centerline
...
Raw data was acquired at a sampling frequency of \SI{32,556}{Hz} using a 24-bit analog-to-digital converter.
The minimum and maximum input was 11 to 136,986 microvolts --- values below or above this range were recorded at the floor or ceiling value respectively.
...
This frequency range extracted the high frequency components of the neural signal whilst excluding high frequency noise from the experimental equipment.
The upper cutoff frequency was manually selected for each channel and session.
...
\item Find the peak of the spike by searching for next time the voltage decreases (searching forwards by at most 24 data points).
...
\end{enumerate}
This algorithm enforces a minimum inter-spike interval of \SI{0.74}{\milli\second}.
