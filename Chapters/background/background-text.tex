...
.\end{align}
Here, we make use of the conditional entropy, $\HH(X | Y)$, so named because it is the entropy of one variable when conditioned on the state of another.
This is also referred to as the noise entropy, particularly when we consider the entropy of the response conditioned on the stimulus, $\HH(R|S)$.
...
% For brevity, we will refer to {mutual information} to as \termemph{information} in the remainder of this thesis.
...
Some noise correlations are inevitable, since there are more neurons in \ac{V1} than in the \ac{LGN} \citep{Kanitscheider2015}.
...
\centering
\hspace*{\fill}
\subfloat[][Tuning curves for two model neurons.\label{fig:FrankeA}]{%
    \includegraphics[scale=1.5]{%
10.1016-j.neuron.2015.12.037Figure6a1.eps}}
\hspace*{\fill}\hspace{.2cm}\hspace*{\fill}
\subfloat[][Pairwise responses traverse a manifold in $2d$ space.\label{fig:FrankeB}]{%
    \includegraphics[scale=1.5]{%
10.1016-j.neuron.2015.12.037Figure6a2.eps}}
\hspace*{\fill}
\\
\subfloat[][.\label{fig:FrankeC}]{%
    \includegraphics[scale=1.5]{%
...
\protect\subref{fig:FrankeA}: Two model direction-selective neurons respond to different stimuli (dashed lines) according to tuning curves (solid grey curves), $f_1(\theta)$ and $f_2(\theta)$, with two direction preferences that differ by \SI{90}{\degree}.
\protect\subref{fig:FrankeB}: The two tuning curves are represented as a solid grey line parametrized by the stimulus direction, $\theta$.
...
\protect\subref{fig:FrankeC}: For each stimulus presented, noise correlation distorts the cloud of two-neuron responses about the mean over trials; depending upon the geometry of this distortion with respect to the informative direction, it can either benefit or harm the coding accuracy.
