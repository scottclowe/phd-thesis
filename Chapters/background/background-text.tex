...
In particular, the amount of information about the stimulus contained in the response is their \termemph{mutual information}, $\I(\SET{S};\SET{R})$.
...
In general, for two random variables $X$ and $Y$, we can express their mutual information as
\begin{align}
\I(\SET{X};\SET{Y})
...
.\end{align}
Here, we make use of the conditional entropy, $\HH(\SET{X} | \SET{Y})$, so named because it is the entropy of one variable when conditioned on the state of another.
This is also referred to as the noise entropy, particularly when we consider the entropy of the response conditioned on the stimulus, $\HH(\SET{R}|\SET{S})$.
...
The Venn diagram shown in \autoref{fig:bg_info_venn} illustrates the relationship between the entropies of $\SET{X}$ and $\SET{Y}$, their joint entropy, conditional entropies, and mutual information, which may assist the reader in conceptualising the relationship between these terms.

\begin{figure}[htbp]
\centering
\includegraphics[scale=.5]{Entropy-mutual-information-relative-entropy-relation-diagram.pdf}
\caption{
\captionemph{Venn diagram of mutual information between $\SET{X}$ and $\SET{Y}$.}
