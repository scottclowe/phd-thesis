...
In humans and other mammals, the visual system is composed of several processing stages, illustrated in \autoref{fig:bg_visual_pathway}.
Light enters the eye (potentially focused into a clear image by the lens), and is encoded as electrical signals in the retina at the back of the eye.
...
The computations performed by the central nervous system allow it to extract features from this stream of sensory information, store properties of it for later computational use, and decide which behavioural actions to perform in order to influence the environment of its body \citep{TEDWolpert}.
...
The change in voltage within one neuron propagates along its cell body, and can affect other neurons which make direct conductive connections with it.
However, the majority of connections between neurons are indirect, involving a synaptic junction between the output of one neuron and the input to the next.
In this situation, the voltage of one neuron (the ``sending'' neuron) --- which is negative at rest, around \SI{-70}{\milli\volt} --- must become sufficiently hyperpolarised to trigger a change in the state of the neuron such the distribution of ion channels open changes.
Then there is a spike and it travels down the axon.
At the terminus of the axon, neurotransmitters are released which cross the synapse to induce a change in voltage in the recipient neuron.
...
%
% Light, after entering the eye, lands upon the retina, where neurons detect and encode.
...
In mammals the light-sensitive cells, or photoreceptors, come in two types: rods and cones.%
\footnote{
...
Rods and cones are subtypes of neuron which are surrounded by photosensitive pigment.
When photons of light collide with the photopigment protein, it changes state and shape, causing a cascade of biochemical changes resulting in the closing of ion channels in the cell membrane of the neuron.
Since the the energy in the photon%
...
(which is indivisibly quantised)
must closely match the difference in energy levels of the photopigment, each photopigment is only sensitive to a particular range of wavelengths of light.
...
Cone photoreceptors come in several different types, using different photopigments to detect different ranges of the electromagnetic spectrum.
In humans, there are three types%
\footnote{With the exception of colour-blind individuals, who sometimes have only two or fewer types of cones, and tetrachromats \citep{Nagy1981,Jordan1993,Jameson2001} who have four.}
of cones:
...
Possessing cones with different spectral sensitivities enables \textemph{colour vision}.
For a given frequency of light, we can consider the response evoked in each cone type.
...
RGCs come in at least 17 types \citep{Nassi2009}

There are three dominant types

Bistratified \acp{RGC} feed Koniocellular \ac{LGN} (K-pathway), then L2/3 \ac{V1} blob cells

Midget \acp{RGC} feed Parvocellular \ac{LGN} (P-pathway), then L$4C\beta$

Parasol \acp{RGC} feed Magnocellular \ac{LGN} (M-pathway), then L$4C\alpha$
...
thought to be a central hub for 


\subsection{Primary visual cortex}

The primary visual cortex is sensitive to orientated bars edges at different

Classically, this was understood as simple and complex cells, however the full processing is more complicated \cite{Harris2013}

Topological maps form across the cortex, which appears to be a self-organising mapping \cite{Miikkulainen2005,Stevens2013,Wilson2015}.


\subsection{Rest of cortex}

Infero-temporal gyrus contains face cells and concept cells \cite{Quiroga2012}
...
Noise correlations are a hindrance (information-limiting correlations, \citet{Moreno-Bote2014}) only when the direction of noise correlation points in the same direction as the derivative of the tuning manifold, since this change is easily confused with a change in the parameter describing the manifold.
