...
When studying neural recordings using information theory, we will need to take note of the uncertainty in which stimulus is presented, $H(\SET{S})$, and the uncertainty in the response, $H(\SET{R})$.
...
\begin{equation}
\I(\SET{S};\SET{R}) = H(\SET{S}) - H(\SET{S} | \SET{R}) = H(\SET{R}) - H(\SET{R} | \SET{S})
.\end{equation}
